...
void init(int n,int **a,int *f,int *u,int *v,long int *fb,int *p,int INF,int *m_p);
void path(int n,int **a,int *f,int *u,int *v,long int *fb,int *rc,int *pi,int *lr,int *uc,int INF,int ii,int *jj_p);
void incr(int *f,long int *fb,int *rc,int j);
...
int apc(int n,int **a,int INF,int *z_p,int *f, double* sum)
{
	...
	*/
	...
	fb = (long int*) malloc(n*sizeof(long int));
	if(fb == NULL){
		free(u); 
		free(v);
		return -1;
	...
	//printf("inicializando declarando...\n");
...
//PHASE 1 .
	...
	//printf("SCANNING OF THE COLUMNS %d, %d\n", n, INF);
